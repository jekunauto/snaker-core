...
public interface IQueryService {
	...
	/**
	 * 根据filter查询所有已完成的任务(与用户表关联查询)
	 * @param filter 查询过滤器
	 * @return List<HistoryTask> 历史任务集合
	 */
	List<HistoryTask> getHistoryRelateTasks(QueryFilter filter);
	
	/**
	 * 根据filter分页查询已完成的历史任务(与用户表关联查询)
	 * @param page 分页对象
	 * @param filter 查询过滤器
	 * @return List<HistoryTask> 历史任务集合
	 */
	List<HistoryTask> getHistoryRelateTasks(Page<HistoryTask> page, QueryFilter filter);
