...
public class SnakerEngineImpl implements SnakerEngine {
	...
	public List<Task> executeTask(String taskId, String operator,
			List<String> outputNames, Map<String, Object> nextOperators,
			Map<String, Object> args) throws SnakerException {
		    //完成任务，并且构造执行对象
			Execution execution = execute(taskId, operator,args);
			if(execution == null) return Collections.emptyList();
			if(null != outputNames && outputNames.size()>0) 
				execution.setOutputNames(outputNames);
			if(null != nextOperators && nextOperators.size()>0) 
	 ...
	 * 2、nodeName不为null时，则任意跳转，即动态创建转移
	 */
	public List<Task> executeAndJumpTask(String taskId, String operator,Map<String, Object> nextOperators,
