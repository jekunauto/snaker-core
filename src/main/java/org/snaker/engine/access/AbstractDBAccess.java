...
public abstract class AbstractDBAccess implements DBAccess {
	...
	protected static final String PROCESS_INSERT = "insert into t_wf_process (id,name,display_Name,type,instance_Url,state,version,create_Time,creator) values (?,?,?,?,?,?,?,?,?)";
	protected static final String PROCESS_UPDATE = "update t_wf_process set name=?, display_Name=?,state=?,instance_Url=?,create_Time=?,creator=? where id=? ";
	protected static final String PROCESS_DELETE = "delete from t_wf_process where id = ?";
	protected static final String PROCESS_UPDATE_BLOB = "update t_wf_process set content=? where id=?";
	protected static final String PROCESS_UPDATE_TYPE = "update t_wf_process set type=? where id=?";
	
	protected static final String ORDER_INSERT = "insert into t_wf_order (id,process_Id,creator,create_Time,parent_Id,parent_Node_Name,expire_Time,last_Update_Time,last_Updator,order_No,variable,version) values (?,?,?,?,?,?,?,?,?,?,?,?)";
	protected static final String ORDER_UPDATE = "update t_wf_order set last_Updator=?, last_Update_Time=?, variable = ?, expire_Time=?, version = version + 1 where id=? and version = ?";
	protected static final String ORDER_DELETE = "delete from t_wf_order where id = ?";
	protected static final String ORDER_HISTORY_INSERT = "insert into t_wf_hist_order (id,process_Id,order_State,creator,create_Time,end_Time,parent_Id,expire_Time,order_No,variable) values (?,?,?,?,?,?,?,?,?,?)";
	protected static final String ORDER_HISTORY_UPDATE = "update t_wf_hist_order set order_State = ?, end_Time = ?, variable = ? where id = ? ";
	protected static final String ORDER_HISTORY_DELETE = "delete from t_wf_hist_order where id = ?";
	
	protected static final String CCORDER_INSERT = "insert into t_wf_cc_order (order_Id, actor_Id, creator, create_Time, status) values (?, ?, ?, ?, ?)";
	protected static final String CCORDER_UPDATE = "update t_wf_cc_order set status = ?, finish_Time = ? where order_Id = ? and actor_Id = ?";
	protected static final String CCORDER_DELETE = "delete from t_wf_cc_order where order_Id = ? and actor_Id = ?";
	
	protected static final String TASK_INSERT = "insert into t_wf_task (id,order_Id,task_Name,display_Name,task_Type,perform_Type,operator,create_Time,finish_Time,expire_Time,action_Url,parent_Task_Id,variable,version) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	protected static final String TASK_UPDATE = "update t_wf_task set finish_Time=?, operator=?, variable=?, expire_Time=?, action_Url=?, version = version + 1 where id=? and version = ?";
	protected static final String TASK_DELETE = "delete from t_wf_task where id = ?";
	protected static final String TASK_HISTORY_INSERT = "insert into t_wf_hist_task (id,order_Id,task_Name,display_Name,task_Type,perform_Type,task_State,operator,create_Time,finish_Time,expire_Time,action_Url,parent_Task_Id,variable) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	protected static final String TASK_HISTORY_DELETE = "delete from t_wf_hist_task where id = ?";
	
	protected static final String TASK_ACTOR_INSERT = "insert into t_wf_task_actor (task_Id, actor_Id) values (?, ?)";
	protected static final String TASK_ACTOR_DELETE = "delete from t_wf_task_actor where task_Id = ?";
	protected static final String TASK_ACTOR_REDUCE = "delete from t_wf_task_actor where task_Id = ? and actor_Id = ?";
	protected static final String TASK_ACTOR_HISTORY_INSERT = "insert into t_wf_hist_task_actor (task_Id, actor_Id) values (?, ?)";
	protected static final String TASK_ACTOR_HISTORY_DELETE = "delete from t_wf_hist_task_actor where task_Id = ?";
	
	protected static final String QUERY_VERSION = "select max(version) from t_wf_process ";
	protected static final String QUERY_PROCESS = "select id,name,display_Name,type,instance_Url,state, content, version,create_Time,creator from t_wf_process ";
	protected static final String QUERY_ORDER = "select o.id,o.process_Id,o.creator,o.create_Time,o.parent_Id,o.parent_Node_Name,o.expire_Time,o.last_Update_Time,o.last_Updator,o.priority,o.order_No,o.variable, o.version from t_wf_order o ";
	protected static final String QUERY_TASK = "select id,order_Id,task_Name,display_Name,task_Type,perform_Type,operator,create_Time,take_Time,finish_Time,expire_Time,action_Url,parent_Task_Id,variable, version from t_wf_task ";
	protected static final String QUERY_TASK_ACTOR = "select task_Id, actor_Id from t_wf_task_actor ";
	protected static final String QUERY_CCORDER = "select order_Id, actor_Id, creator, create_Time, finish_Time, status from t_wf_cc_order ";
	
	protected static final String QUERY_HIST_ORDER = "select o.id,o.process_Id,o.order_State,o.priority,o.creator,o.create_Time,o.end_Time,o.parent_Id,o.expire_Time,o.order_No,o.variable from t_wf_hist_order o ";
	/*protected static final String QUERY_HIST_TASK = "select id,order_Id,task_Name,display_Name,task_Type,perform_Type,task_State,operator,create_Time,finish_Time,expire_Time,action_Url,parent_Task_Id,variable from t_wf_hist_task ";*/
	protected static final String QUERY_HIST_TASK = "select id,order_Id,task_Name,display_Name,task_Type,perform_Type,task_State,operator,create_Time,take_Time,finish_Time,expire_Time,action_Url,parent_Task_Id,variable from t_wf_hist_task ";
	protected static final String QUERY_RELATE_HIST_TASK = "select t.id,t.order_Id,t.task_Name,t.display_Name,t.task_Type,t.perform_Type,t.task_State,u.full_name as operator,t.create_Time,take_Time,t.finish_Time,t.expire_Time,t.action_Url,t.parent_Task_Id,t.variable from t_wf_hist_task t ";
	
	protected static final String QUERY_HIST_TASK_ACTOR = "select task_Id, actor_Id from t_wf_hist_task_actor ";
	
	/**委托代理CRUD*/
	protected static final String SURROGATE_INSERT = "insert into t_wf_surrogate (id, process_Name, operator, surrogate, odate, sdate, edate, state) values (?,?,?,?,?,?,?,?)";
	protected static final String SURROGATE_UPDATE = "update t_wf_surrogate set process_Name=?, surrogate=?, odate=?, sdate=?, edate=?, state=? where id = ?";
	protected static final String SURROGATE_DELETE = "delete from t_wf_surrogate where id = ?";
	protected static final String SURROGATE_QUERY = "select id, process_Name, operator, surrogate, odate, sdate, edate, state from t_wf_surrogate";
	...
	public void updateTaskActor(String taskId,String actor) {
		if(!isORM()) {
			int[] type = new int[]{Types.VARCHAR, Types.VARCHAR};
			saveOrUpdate(buildMap(TASK_ACTOR_DELETE, new Object[]{taskId, actor}, type));
			TaskActor taskActor = new TaskActor();
			taskActor.setTaskId(taskId);
			taskActor.setActorId(actor);
			saveTaskActor(taskActor);
		}
	}
	...
	public List<Task> getNextActiveTasks(String orderId, String taskName, String parentTaskId) {
		String sql = QUERY_TASK + " where parent_task_id in ( select ht.id from t_wf_hist_task ht where ht.order_id=? and ht.task_name=? and ht.parent_task_id=? )";
		return queryList(Task.class, sql, orderId, taskName, parentTaskId);
	}
	
	public HistoryTask getHistRelateTask(String taskId) {
		String where = " ,t_n_user u where u.id=t.operator and t.id = ?";
	...
	public List<Order> getActiveOrders(Page<Order> page, QueryFilter filter) {
        ...
        sql.append(" left join t_wf_process p on p.id = o.process_id ");
		sql.append(" where 1=1 ");
		List<Object> paramList = new ArrayList<Object>();
		if(filter.getOperators() != null && filter.getOperators().length > 0) {
	...
	public List<Task> getActiveTasks(Page<Task> page, QueryFilter filter) {
		...
		if(isFetchActor) {
			sql.append(" left join t_wf_task_actor ta on ta.task_id = id ");
		}
		sql.append(" where 1=1 ");
		List<Object> paramList = new ArrayList<Object>();
	...
	public List<HistoryTask> getHistoryRelateTasks(Page<HistoryTask> page, QueryFilter filter) {
        StringBuilder sql = new StringBuilder(QUERY_RELATE_HIST_TASK);
		boolean isFetchActor = filter.getOperators() != null && filter.getOperators().length > 0;
		sql.append(" left join t_n_user u on u.id = t.operator ");
		if(isFetchActor) {
			sql.append(" left join t_wf_hist_task_actor ta on ta.task_id = t.id ");
		}
		sql.append(" where 1=1 ");
		List<Object> paramList = new ArrayList<Object>();
		if(StringHelper.isNotEmpty(filter.getOrderId())) {
	...
	public List<WorkItem> getWorkItems(Page<WorkItem> page, QueryFilter filter) {
		...
		sql.append(" select distinct o.process_Id, t.order_Id, t.id as id, t.id as task_Id, p.display_Name as process_Name,ff.title as order_Title, p.instance_Url, o.parent_Id, o.creator, ");
		...
		sql.append(" t.create_Time as task_Create_Time,t.take_time as task_Take_Time, t.finish_Time as task_End_Time, t.expire_Time as task_Expire_Time, t.variable as task_Variable,u.full_Name,org.name as org_Name");
		sql.append(" from t_wf_task t ");
		sql.append(" left join t_wf_order o on t.order_id = o.id ");
		sql.append(" left join t_wf_task_actor ta on ta.task_id=t.id ");
		sql.append(" left join t_wf_process p on p.id = o.process_id ");
		//修改 添加与用户表关联|
		sql.append(" left join t_n_user u on o.creator=u.id ");
		//修改 添加与部门表关联
		sql.append(" left join t_n_org org on org.id=u.org_id ");
		//修改 添加与流程表单关联
		sql.append(" left join t_flow_form ff on ff.order_id=o.id ");
		sql.append(" where 1=1 ");
		...
		if(StringHelper.isNotEmpty(filter.getTitle())) {
			sql.append(" and ff.title like ? ");
			paramList.add("%" + filter.getTitle() + "%");
		}
		if(!filter.isOrderBySetted()) {
            filter.setOrder(QueryFilter.DESC);
	...
	public List<HistoryOrder> getCCWorks(Page<HistoryOrder> page, QueryFilter filter) {
		...
		sql.append(" from t_wf_cc_order cc ");
		sql.append(" left join t_wf_hist_order o on cc.order_id = o.id ");
		sql.append(" where 1=1 ");
	...
	public List<WorkItem> getHistoryWorkItems(Page<WorkItem> page, QueryFilter filter) {
		...
		sql.append(" t.display_Name as task_Name, t.task_Name as task_Key, t.task_Type, t.perform_Type,t.operator, t.action_Url, ");
		sql.append(" t.create_Time as task_Create_Time,t.take_time as task_Take_Time, t.finish_Time as task_End_Time, t.expire_Time as task_Expire_Time, t.variable as task_Variable,u.full_Name,org.name as org_Name ");
		sql.append(" from t_wf_hist_task t ");
		sql.append(" left join t_wf_hist_order o on t.order_id = o.id ");
		sql.append(" left join t_wf_hist_task_actor ta on ta.task_id=t.id ");
		sql.append(" left join t_wf_process p on p.id = o.process_id ");
		//修改 添加与用户表关联
		sql.append(" left join t_n_user u on o.creator=u.id ");
		...
		if(filter.getOperators() != null && filter.getOperators().length > 0) {
			...
			//TODO 业务需求（一个实例只出现一个已办任务）
			StringBuilder paramStr = new StringBuilder();
			List<Object> paramListTmp = new ArrayList<Object>();
			for(String actor : filter.getOperators()) {
				paramStr.append("?,");
				paramListTmp.add(actor);
			}
			paramList.addAll(paramListTmp);
			paramStr.deleteCharAt(paramStr.length() - 1);
			sql.append(" and t.id in(select tmp.id from (select tp.id,tp.ORDER_ID from t_wf_hist_task tp left join t_wf_hist_task_actor tha on tha.task_id=tp.id where ");
			sql.append("(tha.actor_Id in ("+paramStr+")  or tp.OPERATOR in ("+paramStr+")) group by tp.id,tp.ORDER_ID) tmp ");
			sql.append("where tmp.id=(select tmp2.id from (");
			sql.append("select tp.id,tp.ORDER_ID,tp.FINISH_TIME from t_wf_hist_task tp left join t_wf_hist_task_actor tha on tha.task_id=tp.id where ");
			sql.append("(tha.actor_Id in ("+paramStr+")  or tp.OPERATOR in ("+paramStr+")) group by tp.id,tp.ORDER_ID,tp.FINISH_TIME) tmp2 ");
			sql.append("where tmp2.order_id=tmp.order_id order by tmp2.FINISH_TIME desc limit 1) ");
			for (int i = 0; i < 3; i++) {
				paramList.addAll(paramListTmp);
			}
			sql.append(") ");
			//sql.append(" (tha.actor_Id in (");
			//andWhere.append(" or ht.OPERATOR in (");
			//for(String actor : filter.getOperators()) {
			//	sql.append("?,");
			//	andWhere.append("?,");
			//	paramList.add(actor);
			...
			//}
			/*sql.deleteCharAt(sql.length() - 1);
			andWhere.delete(andWhere.length()-1, andWhere.length());
			andWhere.append(")");
			sql.append(") ");
			sql.append(andWhere.toString()+") ");
			sql.append(" order by ht.FINISH_TIME desc) tmp group by ,tmp.ORDER_ID ) tmp2) ");*/
			/*
			sql.append(" and (ta.actor_Id in (");
			andWhere.append(" or t.OPERATOR in (");
			for(String actor : filter.getOperators()) {
				sql.append("?,");
				andWhere.append("?,");
				paramList.add(actor);
