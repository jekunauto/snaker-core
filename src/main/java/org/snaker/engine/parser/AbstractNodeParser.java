...
public abstract class AbstractNodeParser implements NodeParser {
	...
	public void parse(Element element) {
		...
		for(Element te : transitions) {
			...
			transition.setType(te.getAttribute(ATTR_TYPE));
			int num = 0;
			int isCheckForm = 1;
			try {
				num = Integer.parseInt(te.getAttribute(ATTR_SORT_NUM));
				isCheckForm = Integer.parseInt(te.getAttribute(ATTR_IS_CHECK_FORM));
			} catch(Exception e) {}
